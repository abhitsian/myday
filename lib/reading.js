...
function build(days = 14) {
  ...
  const add = (key, { title, host, url, source, app, ts }) => {
    const r = items[key] || (items[key] = { key, title, host: host || '', url: url || '', source, apps: {}, secs: 0, days: new Set(), last: 0 });
    if (url && !r.url) r.url = url;
  ...
  for (const v of B.visitsBetween(start, end, S.readConfig())) {
    ...
    add(host + path.slice(0, 40), { title: v.title, host, url: v.url, source: 'web', app: 'browser', ts: v.ts });
  }
  ...
  const list = Object.values(items)
    .map((r) => ({
      title: r.title, host: r.host, url: r.url || '', source: r.source,
