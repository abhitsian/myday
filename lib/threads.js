...
// thread tells you the Karuna migration ran across six days and has been quiet for three,
...
function featurize(days) {
  ...
  for (const date of lastNDates(days)) {
    for (const e of S.readEntries(date)) {
      ...
      if (f.size) notes.push({ entry: e, date, f });
    }
  }
  return notes;
}
...
/// Chrome and "servicenow" are in most of the day, so they connect everything to everything.
...
function nameOf(c) {
  ...
  const proj = top.find(([k]) => k.startsWith('p:'));
  if (proj) return titleCase(proj[0].slice(2));

  if (c.members && c.members.length) {
    let best = null, bestSim = -1;
    for (const m of c.members) {
      const sim = cosine(c.centroid, m.f);
      if (sim > bestSim && m.entry.title) { bestSim = sim; best = m; }
    }
    if (best) {
      ...
      return best.entry.title.split(/[;·]| — |, (?:then|and) /)[0].trim().slice(0, 58);
    }
  }
...
function build(days = 21, opts = {}) {
  ...
  .sort((a, b) => (b.established - a.established) || b.days - a.days || b.minutes - a.minutes);
