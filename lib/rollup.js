...
function localEntry(w) {
  ...
  const title = dominant ? clip(`${dominant.app} — ${dominant.titles[0]}`)
...
async function summarize(w, cfg) {
  ...
  if (cfg.summarizer === 'ondevice') {
    try { return viaOnDevice(w); }
    catch (e) { return localEntry(w).replace('generator: local', `generator: local (ondevice ${String(e.message).slice(0, 30)})`); }
  }
  try {
