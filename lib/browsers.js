...
//
// This is what lets myday say "you were reading the Stripe webhook docs" instead of
...
function query(dbPath, sql) {
  ...
  const tmp = path.join(os.tmpdir(), `myday-${path.basename(path.dirname(dbPath))}-${process.pid}.db`);
  try {
...
function visits(date, cfg) {
  return visitsBetween(+new Date(`${date}T00:00:00`), +new Date(`${date}T00:00:00`) + 864e5, cfg);
}

/// Visits across an arbitrary range, in one pass per browser.
///
/// The per-day helper copies and queries a history database that can be a hundred megabytes.
/// Calling it once per day for a three-week report meant twenty-one copies and twenty-one
/// queries, which took long enough that the page looked hung. Range callers use this.
function visitsBetween(startMs, endMs, cfg) {
...
module.exports = { visits, visitsBetween, installed, CHROMIUM, SAFARI };
