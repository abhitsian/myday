...
function appsDay(date) {
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    // Per-day composition, not just the total: the Apps view stacks it, so you can see the
    // mix shift across the week rather than only whether the day was long.
    weekDays.push({
      date: ds,
      secs: Math.round(w.total),
      byApp: Object.fromEntries(Object.entries(w.byApp).map(([a, x]) => [a, Math.round(x)])),
    });
  }
...
function browseDay(date, cfg = S.readConfig()) {
  ...
  for (const v of visits) {
    ...
    if (last && last.host === v.host && v.ts - last.endTs < 25 * 60000) {
      ...
      // Carry the address with the title so the page can be reopened. Deduped on the
      // TITLE, because the same page arrives under several URLs once tracking parameters
      // are attached, and for a reading list those are one thing you read.
      if (!last.pages.some((p) => p.title === v.title) && last.pages.length < 12) {
        last.pages.push({ title: v.title, url: v.url });
      }
    } else {
      clusters.push({ host: v.host, browser: v.browser, startTs: v.ts, endTs: v.ts,
                      count: 1, pages: [{ title: v.title, url: v.url }] });
    }
  }
  ...
  return {
    ...
    blocks: clusters.map((c) => ({
      ...
      count: c.count,
      pages: c.pages,
      titles: c.pages.map((p) => p.title),   // kept for anything reading the old shape
    })),
  };
}
...
// Title patterns borrowed from Claude Desk, which has had longer to learn what a first
// message actually looks like: a slash command hides inside <command-name> tags, and a
// pasted article can run for pages before the actual question.
function sessionTitle(text) {
  ...
  for (const raw of t.split('\n')) {
    ...
    // A pasted page announces itself; the user's own question sits before it.
    if (/(skip to main|sign in|search for|©|posted on|cookie policy|subscribe)/i.test(line)) break;
    out.push(line); n += line.length;
    if (n > 120) break;
  }
  const title = out.join(' ').trim() || t;
  return title.length > 90 ? title.slice(0, 87) + '…' : title;
}


// Reading every transcript in full, on every view, was the single slowest thing the app did.
// On this machine 1,544 of them carry today's mtime and total 150MB, so opening Today meant
// parsing 150MB of JSON before anything rendered — about seven seconds, on the landing view.
//
// A transcript's contents for a given day are fixed once the file stops changing, so the scan
// is keyed on (mtime, size) and only genuinely-changed files are re-read. The result covers
// every date the file touches, because a session that runs past midnight belongs to two.
const SCAN_V = 1;
const scanMem = new Map();

function scanCachePath() { return path.join(S.ROOT, 'cache', `sessions-v${SCAN_V}.json`); }

let scanDisk = null;
function loadScanDisk() {
  if (scanDisk) return scanDisk;
  try { scanDisk = JSON.parse(fs.readFileSync(scanCachePath(), 'utf8')); } catch { scanDisk = {}; }
  return scanDisk;
}
let scanDirty = false;
let scanPruned = false;
function saveScanDisk() {
  if (!scanDirty || !scanDisk) return;
  try {
    // Drop entries whose transcript is gone. Without this the cache keeps the opening prompt
    // of a session the person deleted from ~/.claude, which is a copy of something they
    // removed. Once per process is enough; the check is a stat per file.
    if (!scanPruned) {
      for (const fp of Object.keys(scanDisk)) if (!fs.existsSync(fp)) delete scanDisk[fp];
      scanPruned = true;
    }
    fs.mkdirSync(path.dirname(scanCachePath()), { recursive: true });
    fs.writeFileSync(scanCachePath(), JSON.stringify(scanDisk));
    scanDirty = false;
  } catch {}
}

function scanTranscript(fp, st) {
  const sig = `${Math.round(st.mtimeMs)}:${st.size}`;
  const mem = scanMem.get(fp);
  if (mem && mem.sig === sig) return mem;

  const disk = loadScanDisk();
  if (disk[fp] && disk[fp].sig === sig) { scanMem.set(fp, disk[fp]); return disk[fp]; }

  const rec = { sig, cwd: '', byDate: {} };
  try {
    for (const ln of fs.readFileSync(fp, 'utf8').split('\n')) {
      if (!ln.trim().startsWith('{')) continue;
      let e; try { e = JSON.parse(ln); } catch { continue; }
      if (e.cwd && !rec.cwd) rec.cwd = e.cwd;
      const ts = e.timestamp ? +new Date(e.timestamp) : 0;
      if (!ts) continue;
      const dkey = S.isoDate(new Date(ts));
      const d = rec.byDate[dkey] || (rec.byDate[dkey] = { tsMin: null, tsMax: null, turns: 0, firstUser: '' });
      d.turns++;
      if (!d.tsMin || ts < d.tsMin) d.tsMin = ts;
      if (!d.tsMax || ts > d.tsMax) d.tsMax = ts;
      if (!d.firstUser && e.type === 'user') {
...
function sessionsDay(date) {
  // Honour the source switch here rather than only in the UI. A toggle that the API ignores
  // is not a setting, and the timeline was still showing sessions after Claude Code was
  // switched off.
  const cfg = S.readConfig();
  if ((cfg.sources || {}).claudeCode === false) {
    return { date, available: false, sessions: [], disabled: true };
  }
  ...
  for (const proj of projects) {
    ...
    for (const f of files) {
      ...
      const scanned = scanTranscript(fp, st);
      if (!scanned) continue;
      const day = scanned.byDate[date];
      const cwd = scanned.cwd;
      if (!day) continue;
      const { tsMin, tsMax, turns, firstUser } = day;
      ...
      out.push({
        ...
        prompt: firstUser || '(no prompt captured)',
        title: sessionTitle(firstUser || ''),
        // The filename is the session id, which is what makes a session navigable rather
        // than merely visible: `claude --resume <id>` puts you back inside it.
  ...
  // Collapse automation. A prompt that repeats many times a day in one-minute bursts is a
  // scheduled job, not you working — and the worst offender was myday's own rollup, which
  // opens "You write one entry in a personal computer-history log" every ten minutes and
  // filled the view with 100+ identical rows. Detected by shape rather than by matching
  // known prompts, so anyone's cron jobs collapse the same way.
  const byPrompt = {};
  for (const s of out) {
    const k = (s.prompt || '').slice(0, 60);
    (byPrompt[k] = byPrompt[k] || []).push(s);
  }
  const automated = new Set();
  const jobs = [];
  for (const [k, group] of Object.entries(byPrompt)) {
    if (group.length < 5) continue;
    const median = group.map((g) => g.mins).sort((a, b) => a - b)[Math.floor(group.length / 2)];
    if (median > 2) continue;
    group.forEach((g) => automated.add(g));
    jobs.push({ prompt: k, runs: group.length, project: group[0].project,
                mins: group.reduce((a, g) => a + g.mins, 0) });
  }
  const worked = out.filter((s) => !automated.has(s));
  ...
  const byProject = {};
  for (const s of worked) (byProject[s.project] = byProject[s.project] || []).push(s);

  // One write per pass rather than one per file.
  saveScanDisk();
  ...
  return {
    ...
    totalMins: union(worked),
    spanMins: worked.reduce((a, s) => a + s.mins, 0),
    // Reported rather than silently dropped: a view that quietly hides rows is worse than
    // one that says what it set aside.
    automation: jobs.sort((a, b) => b.runs - a.runs),
  };
}

module.exports = { appsDay, browseDay, sessionsDay, sessionTitle };
...
function homeDay(date, cfg = S.readConfig()) {
  ...
  // Round to whole minutes first, then split. Splitting first let 9h 59m 59s round its
  // remainder up to 60 and print "9h 60m".
  const dur = (s) => {
  ...
  if (sessions.available && sessions.projects.length) {
    ...
    insights.push({ kind: 'info', icon: 'code',
      text: `Most-worked project: **${p.project}**, ${dur(p.mins * 60)} across ${p.count} session${p.count === 1 ? '' : 's'}.` });
  }
  ...
  if (apps.first && apps.last) {
    ...
    // Each sample is credited a fixed step, so on a short day the credited seconds can
    // exceed the wall-clock span and produce "present for 102%".
    const density = span > 0 ? Math.min(100, Math.round(apps.active / span * 100)) : 0;
